...
class TestGeminiLLMInterface:
    ...
    def test_perform_technical_research_no_json_response(self, mock_api_key):
        """Test technical research when response doesn't contain JSON"""
        with patch('llm_interface.genai') as mock_genai:
            # Set up mock to return plain text without JSON
            mock_model_instance = MagicMock()
            mock_content = MagicMock()
            mock_content.text = "Plain text response without JSON structure"
            mock_genai.GenerativeModel.return_value = mock_model_instance
            mock_model_instance.generate_content.return_value = mock_content
            ...
            assert result["findings"] == "Plain text response without JSON structure"
            assert result["sources"] == ["Generated by Gemini LLM"]
            assert result["confidence"] == 0.7
            assert result["timestamp"] == "2023-10-01T10:00:00Z"

    def test_perform_economic_research_no_json_response(self, mock_api_key):
        """Test economic research when response doesn't contain JSON"""
        with patch('llm_interface.genai') as mock_genai:
            # Set up mock to return plain text without JSON
            mock_model_instance = MagicMock()
            mock_content = MagicMock()
    ...
    def test_perform_social_cultural_research_no_json_response(self, mock_api_key):
        """Test social/cultural research when response doesn't contain JSON"""
        with patch('llm_interface.genai') as mock_genai:
            # Set up mock to return plain text without JSON
            mock_model_instance = MagicMock()
            mock_content = MagicMock()
            ...
            result = llm_interface.perform_social_cultural_research("test query", "test context")
    ...
    def test_perform_social_cultural_research_error_handling(self, mock_api_key):
        """Test social/cultural research handles API errors gracefully"""
        with patch('llm_interface.genai') as mock_genai:
            # Set up mock to raise exception
            mock_model_instance = MagicMock()
            mock_genai.GenerativeModel.return_value = mock_model_instance
            ...
            assert "findings" in result
            assert "Error in social/cultural analysis: API Error" in result["findings"]
            assert result["confidence"] == 0.0