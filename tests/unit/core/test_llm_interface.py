...
class TestGeminiLLMInterface:
    ...
    @pytest.mark.parametrize(
        ...
        [
            ("perform_technical_research", "technical analysis"),
            ("perform_economic_research", "economic analysis"),
            ("perform_social_cultural_research", "social/cultural analysis"),
        ],
    )
    def test_perform_research_error_handling(self, mock_api_key, method_name, error_prefix):
        """Ensure each research path surfaces API errors consistently"""
        with patch('llm_interface.genai') as mock_genai:
            ...
            llm_interface.model = mock_model_instance

            method = getattr(llm_interface, method_name)
            result = method("test query", "test context")

            assert "findings" in result
            assert f"Error in {error_prefix}: API Error" in result["findings"]
    ...
    @pytest.mark.parametrize(
        "method_name",
        [
            "perform_technical_research",
            "perform_economic_research",
            "perform_social_cultural_research",
        ],
    )
    def test_perform_research_no_json_response(self, mock_api_key, method_name):
        """Ensure each research path falls back cleanly when JSON is missing"""
        with patch('llm_interface.genai') as mock_genai:
            mock_model_instance = MagicMock()
