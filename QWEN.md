...
- `main.py`: Main entry point with orchestrator factory pattern
- `a2a_protocol.py`: Defines the Agent-to-Agent communication protocol
- `llm_interface.py`: Interface to interact with Google's Gemini LLM
- `agents/`: Individual agent implementations with specialized functionality
- `tools/`: Tool framework and specific tool implementations
- `tests/`: Comprehensive test suite covering all system functionality
- `development-phases.md`: Detailed project roadmap and completed milestones
- `multi-agent-system-spec.md`: Technical specifications for the system
- `product-definition.md`: Product requirements and feature definitions

## Development Conventions
- Each agent implements the A2A protocol for standardized communication
- All communication between agents follows the A2A protocol specification
- Agents have comprehensive error handling and retry mechanisms
- Logging is implemented across all agents for monitoring and debugging
- Tests follow the unit testing pattern with high coverage (97%+)
- Tools follow the Tool framework interface and are independently manageable
- Each agent has its own directory with dedicated documentation

## Development Phases
### Phase 1: MVP (Completed)
- Created core agents (Research Orchestrator, Tech, Economic, Fact-checking)
- Implemented basic A2A protocol
- Basic tool framework and integration
- Autonomous tool discovery and selection

### Phase 2: Enhanced Functionality (Completed)
- Added social/cultural agent
- Improved output formatting and structure
- Implemented error handling and retry mechanisms
- Added logging and monitoring
- Added support for multiple orchestrator types
- Comprehensive test coverage (97%)

### Future Phases
- Phase 3: Enhanced robustness, reliability, and JSON-based workflow configuration
- Phase 4: Real tool implementations (currently using mock implementations)
- Phase 5: Security enhancements
- Phase 6: User experience improvements
- Phase 7: Advanced analytics and visual orchestrator builder
